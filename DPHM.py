...
from pm4py.algo.discovery.heuristics import algorithm as heuristics_converter
...
class DPHM:
    ...
    def __init__(self, gui):
        # Hyperparameters
        self.max_sampling_tries: int = 500
    ...
    def noise_matrix(self):
        ...
        for key, value in self.matrix.items():
            noised_matrix[key] = int(self.add_laplace_noise(value, 1, self.GUI.epsilon.get() * self.e0))
    ...
    def rejection_sampling(self, renoise: bool=True):
        ...
        for i in range(0, self.max_sampling_tries):
            ...
            if not self.is_weakly_connected_dfg(self.noised_heu_net.dfg):
                continue
            ...
            try:
                ...
                net, initial_marking, final_marking = self.convert_heuristics_net_to_petri_net()
    ...
    def convert_heuristics_net_to_petri_net(self):
        return heuristics_converter.classic.hn_conv_alg.apply(self.noised_heu_net)
    ...
    @staticmethod
    def is_weakly_connected_dfg(dfg):
        nodes = set()
        graph = {}
    ...
    def check_rejection(self) -> bool:
            ...
            if rej_sam_attr == "Fitness":
                try:
                    ...
                    if self.add_laplace_noise(fitness_tb.get('log_fitness'), 1 / len(self.trace_list), self.GUI.epsilon.get() * self.e1) >= thresh_value:
                        self.render()
                        return True
                except ValueError:
                    pass
                ...
                try:
                    ...
                    if self.add_laplace_noise(precision_tb, 1/len(self.trace_list), self.GUI.epsilon.get() * self.e1) >= thresh_value:
                        self.render()
                        return True
                except ValueError:
                    pass
                ...
                try:
                    ...
                    if self.add_laplace_noise(simplicity, 1/len(self.trace_list), self.GUI.epsilon.get() * self.e1) >= thresh_value:
                        self.render()
                        return True
                except ValueError:
                    pass
                ...
                try:
                    ...
                    if self.add_laplace_noise(generalization, 1/len(self.trace_list), self.GUI.epsilon.get() * self.e1) >= thresh_value:
                        self.render()
                        return True
                except ValueError:
                    pass
                ...
                try:
                    fitness_tb = pm4py.fitness_token_based_replay(self.event_log, self.net, self.im, self.fm).get('log_fitness')
                    precision_tb = pm4py.precision_token_based_replay(self.event_log, self.net, self.im, self.fm)
                    f1 = 0 if (fitness_tb + precision_tb) == 0 else 2 * fitness_tb * precision_tb / (fitness_tb + precision_tb)

                    if self.add_laplace_noise(f1, 1/len(self.trace_list), self.GUI.epsilon.get() * self.e1) >= thresh_value:
                        self.render()
                        return True
                except ValueError:
                    pass

            elif rej_sam_attr == "No Rejection Sampling":
                self.render()
                return True
